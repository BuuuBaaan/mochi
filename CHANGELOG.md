...
- P5インスパイアの再設計を実施し、濃紺ベース + アクア層 + 限定レッドCTAのデザインシステムへ更新
- `theme.config.ts` と `app/globals.css` のテーマ変数を刷新（色/角丸/影/罫線/余白/モーション速度）
- ヘッダー/フッター/CTA/カード/バッジ/ページヒーローを斜めカット・ステッカー感・高コントラスト仕様へ統一
- ホームのファーストビューを再構成し、ロゴ主役化 + 主要CTA（ヤフオク/BASE/YouTube/Instagram/物販Amazon）を最短配置
- 物販タブUIを刷新し、商品タブ（大）・クイックタブ（中）で泡エフェクトの粒度を分離
- モバイル向けに `hover` 非依存の `:active` 代替演出を追加し、演出の破綻を防止
- `README.md` の `Design References` にP5/Atlus関連リファレンスを8件追記し、`P5 UI再構成メモ` を追加
- ダーク基調でも可読性を維持するため、`site-surface` 配下のテキスト/背景/入力UIコントラストを再調整
