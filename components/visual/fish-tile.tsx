...
export function FishTile({
  ...
  return (
    <div
      aria-label={label}
      className={cn(
        "media-placeholder relative flex h-40 items-center justify-center overflow-hidden rounded-2xl border border-white/20 shadow-[0_12px_24px_rgba(2,6,23,0.22)]",
        className,
      )}
    >
      {imageSrc ? (
        <Image src={imageSrc} alt={label} fill className="object-cover object-center opacity-84" />
      ) : null}
