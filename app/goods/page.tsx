          ...
          <div className="grid gap-3 md:grid-cols-2">
            {goodsProducts.map((product) => (
              <Link
                ...
                className="bubble-hover rounded-2xl border border-sky-300/70 bg-white/75 px-4 py-3 transition hover:-translate-y-0.5 hover:bg-white"
              >
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-cyan-800">注目商品</p>
                <p className="mt-1 text-lg font-semibold text-slate-900">{product.name}</p>
                <p className="mt-1 text-sm leading-7 text-slate-700">{product.shortCatch}</p>
              </Link>
            ))}
          </div>
          <div className="flex flex-wrap gap-2">
            {goodsQuickTabs.map((tab) => (
              <Link
                key={tab.href}
                href={tab.href}
                className="bubble-hover rounded-full border border-sky-300/75 bg-white/75 px-4 py-2 text-sm font-semibold text-cyan-900 transition hover:bg-white"
              >
                {tab.label}
              </Link>
            ))}
