...
export async function GET(request: Request) {
  ...
  const subtitle = searchParams.get("subtitle") ?? siteConfig.heroTagline;
    ...
    (
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
          <div style={{ fontSize: 70, fontWeight: 800, lineHeight: 1.12, maxWidth: "980px" }}>{title}</div>
          <div style={{ fontSize: 34, opacity: 0.96 }}>{subtitle}</div>
        </div>
      </div>
    ),
    {
